...
export default async function BroadcastHistoryPage() {
  ...
  const broadcastList = getAllBroadcasts();
    ...
    <div className="container py-24 max-w-7xl space-y-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12 border-b-8 border-black pb-12">
        <div className="space-y-4 border-l-8 border-black pl-8">
          ...
          <p className="text-sm font-black uppercase tracking-[0.4em] text-mono-400">Archival record of institutional dispatches</p>
        </div>
                ...
                {broadcastList.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="h-64 text-center">
                      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-mono-400">NO TRANSMISSION RECORDS DETECTED IN ARCHIVE.</p>
                    </TableCell>
                  </TableRow>
                ) : (
                  broadcastList.map((log) => (
                    <TableRow key={log.id} className="border-b-2 border-black/5 last:border-none hover:bg-mono-50 transition-colors">
                      <TableCell className="pl-10 py-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] font-bold text-mono-400">{new Date(log.created_at).toLocaleTimeString('en-GB')}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="max-w-md">
                        <div className="space-y-1">
                          ...
                          {log.subject && <p className="text-[9px] font-bold text-mono-400 uppercase truncate">{log.body.substring(0, 60)}...</p>}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="space-y-1">
                      ...
                      <TableCell>
                        <Badge className={`rounded-none px-3 py-1 text-[9px] font-black uppercase tracking-widest border-2 ${log.status === 'sent'
                            ...
                            : log.status === 'failed'
                              ...
                              : 'bg-white text-mono-400 border-mono-200'
                          }`}>
          ...
          <Card className="border-4 border-black rounded-none shadow-none bg-white">
            ...
            <CardContent className="p-8 space-y-10">
              <div className="space-y-2">
                <p className="text-[9px] font-black uppercase tracking-widest text-mono-400">TOTAL DISPATCHES</p>
                <p className="text-5xl font-black tracking-tighter">{broadcastList.length}</p>
              </div>
              <div className="space-y-2">
                <p className="text-[9px] font-black uppercase tracking-widest text-mono-400">NODES REACHED</p>
